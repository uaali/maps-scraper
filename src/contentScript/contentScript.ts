...
const scraper = async () => {
  ...
  if (window.location.href.startsWith("https://www.google.com/maps/")) {
    ...
    parents.forEach((parent) => {
      ...
      const ratingTextElement = parent.querySelector(
        "span.fontBodyMedium > span" || null
      );
      const ratingText = ratingTextElement
        ? ratingTextElement.getAttribute("aria-label")
        : null;

      const parentElement = parent as HTMLElement; // Cast to HTMLElement
      ...
      let business = {
        ...
        stars:
          ratingText && ratingText.split("stars")[0].trim()
            ? Number(ratingText.split("stars")[0].trim())
            : null,
        numberOfReviews:
          ratingText && ratingText.split("stars")[1]
